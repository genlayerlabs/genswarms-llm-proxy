...
subset of `Genswarms.LlmProxy.Store` (six optional callbacks: record_llm_call,
record_llm_budget_origin, llm_usage_for_budget, llm_usage_today,
llm_usage_by_budget, list_llm_usage). Missing callbacks fall back to memory —
fail-open by design. The host owns the schema/migrations.
...
`./checks/run.sh` — 11 standalone checks (no PG, no network; injected seams:
